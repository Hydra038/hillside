...
export default function PaymentSettingsManagement() {
  ...
  const handleEdit = (method: PaymentMethod) => {
    ...
    setFormData({
      type: method.type || '',
      displayName: typeof method.displayName === 'string' ? method.displayName : '',
      description: method.description || '',
      enabled: typeof method.enabled === 'boolean' ? method.enabled : true,
      config: {
        accountName: (method.config && method.config.accountName) || '',
        accountNumber: (method.config && method.config.accountNumber) || '',
        sortCode: (method.config && method.config.sortCode) || '',
        reference: (method.config && method.config.reference) || '',
        email: (method.config && method.config.email) || '',
        instructions: (method.config && method.config.instructions) || ''
      }
    });
    setShowAddForm(true);
  };
