...
async function setupDatabase() {
  ...
  if (!process.env.DATABASE_URL) {
    ...
    console.log('Available env vars:', Object.keys(process.env).filter(key => key.startsWith('DATABASE')));
    process.exit(1);
  }
  ...
  try {
    ...
    const [paymentSettings] = await connection.execute('SELECT COUNT(*) as count FROM payment_settings');
